...
describe('computeUsageSummary', () => {
  ...
  it('aggregates per user (tokens over ok rows only; errors counted; sorted by tokens desc)', () => {
    ...
    expect(byUser).toEqual([
      { userId: 'u2', username: 'bob', requests: 1, tokens: 200, promptTokens: 100, completionTokens: 100, errors: 0, cost: null, costIncomplete: false },
      { userId: 'u1', username: 'alice', requests: 3, tokens: 50, promptTokens: 15, completionTokens: 35, errors: 1, cost: null, costIncomplete: false },
    ]);
  });
  ...
  it('aggregates per model (error row contributes a request but 0 tokens)', () => {
    ...
    expect(byModel).toEqual([
      { modelNormalized: 'gpt', requests: 3, tokens: 250, promptTokens: 115, completionTokens: 135, cost: null },
      { modelNormalized: 'claude', requests: 1, tokens: 0, promptTokens: 0, completionTokens: 0, cost: null },
    ]);
  });

  it('computes totals (tokens=ok only, distinct users, error count)', () => {
    expect(computeUsageSummary(db).totals).toEqual({ requests: 4, tokens: 250, errors: 1, users: 2, cost: null, costIncomplete: false });
  });

  it('honours the from/to date filter', () => {
    const { totals } = computeUsageSummary(db, { from: '2026-07-02' }); // excludes row 1
    expect(totals).toEqual({ requests: 3, tokens: 220, errors: 1, users: 2, cost: null, costIncomplete: false });
  });

  it('honours the kind filter', () => {
    const { totals } = computeUsageSummary(db, { kind: 'image' }); // none match
    expect(totals).toEqual({ requests: 0, tokens: 0, errors: 0, users: 0, cost: null, costIncomplete: false });
  });
});
