...
> **Last Updated**: 2026-07-21 (v0.7.36 — Phase 2 data-safety: **versioned schema migrations** (`schema_migrations` ledger + transactional `runMigrations`, replaces the swallow-all `try/catch ALTER` stack; v1 baseline is idempotent so it absorbs both existing prod DBs and fresh ones; new changes append as v2+) + **online DB backups** (native `db.backup()` snapshots, keep-N rotation, `unref`'d daily job env-tunable via `BACKUP_*`, admin `GET`/`POST /api/backups`); +21 tests → 106; see §12 change log)
