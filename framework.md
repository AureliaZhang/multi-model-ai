...
> **Last Updated**: 2026-07-23 (v0.7.41 — **Phase 4 start (FE-A): message actions** — copy / regenerate / edit-and-resend on `MessageBubble` (hover action bar + inline edit). Backed by a new `POST /api/conversations/:id/truncate` (rowid-exact, owner-gated, `truncateMessagesFrom` +5 tests → server 132) that the client uses to truncate the turn then re-send via the existing chat path (no changes to the streaming handler). Regenerate re-sends the prompting user text; edit resends the new text; both drop that turn's attachments (MVP) and discard later turns (standard truncate-and-regenerate). client `tsc -b`+`vite build`+eslint (0 err/0 warn) clean.)
...
- **Phase 4 — "nice to use together":** shared prompt/persona library (FE-B), team-vs-private file library (FE-B + TC0 #4), message actions copy/regenerate/edit (FE-A), conversation search (FE-A). Effort: M–L (splittable). **🟡 In progress:** message actions copy/regenerate/edit **✅ done (v0.7.41)**. Remaining: conversation search, shared prompt/persona library, team-vs-private file library.
