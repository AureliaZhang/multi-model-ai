...
- **Phase 5 — polish:** TC2 perf (chat N+1 + historical re-parse, history `LIMIT`, vector-scan bound, dedupe embeddings), retention purge (TC1 #5), member invite/onboarding (FE-B), chat organize/pin/folders (FE-A), a11y/keyboard/ARIA. Effort: many small. **🟡 In progress:** chat-path perf **✅ done (v0.7.45)** — batched attachment load (killed the N+1), `attachments.extracted_text` cache (migration v5) so historical PDFs/text files parse once not every turn, and one shared query embedding across file-RAG + memory search. **Remaining:** history `LIMIT` (product call — how much context to keep), vector-scan bound, retention purge, invite/onboarding, chat organize, a11y.
