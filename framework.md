...
> **Last Updated**: 2026-07-22 (v0.7.39 — Phase 3 cont.: **per-user monthly token quota (backend + enforcement)** — **schema migration v2** (first real incremental migration on Phase 2's runner) adds `users.monthly_token_limit` (0 = unlimited); `services/quota.ts` (this-month token sum + `checkUserQuota`); `chat.ts` hard-blocks over-quota non-admins (HTTP 429) before send; `users` API exposes + sets the limit. +6 tests → server 127; tsc clean. **Admin UI to set limits = remaining Phase 3 piece** (backend is functional now via the users API; limits default to unlimited so nothing changes until one is set).)
...
| Per-user quota / budget (UI + backend) | settings + `chat` guard | M+M | high | Monthly token cap per member; pairs with §10.8 TC1 rate/quota. **🟡 Backend done (v0.7.39)** — migration v2 `users.monthly_token_limit` (0=unlimited), `services/quota.ts` `checkUserQuota` (this-month `total_tokens` sum), `chat.ts` 429 for over-quota non-admins, `users` API get/set. **Admin UI to set per-member limits still todo** (backend settable via `PUT /api/users/:id { monthlyTokenLimit }`). |
...
- **Phase 3 — "see & control spend":** admin usage + **cost** dashboard (FE-B; data already in `api_usage_logs`), per-user quota/budget (FE-B + TC1), API-key encryption at rest (TC1 #3). Effort: M. **🟡 In progress:** API-key encryption **✅ done (v0.7.37)**; usage dashboard (token-based) **✅ done (v0.7.38)**; per-user monthly token quota **backend ✅ done (v0.7.39)**. **Remaining:** quota admin UI (set per-member limits) + optional $ cost on the dashboard (needs pricing source).
