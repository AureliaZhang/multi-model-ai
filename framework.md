...
> **Last Updated**: 2026-07-23 (v0.7.40 — **Phase 3 COMPLETE**: quota admin UI — `UserManagement` gets an inline `QuotaCell` (Gauge + number input + save/cancel) under each non-admin member to set `monthlyTokenLimit` via `PUT /api/users/:id`; client `UserPublic`/`UpdateUserRequest` carry the field; `users.quota*` i18n (zh/en). client `tsc -b`+`vite build`+eslint (0 err) clean. **Phase 3 done** (API-key encryption ✅ + usage dashboard ✅ + per-user quota backend+UI ✅); only the optional **$ cost** on the dashboard is deferred (owner chose usage-first; needs a pricing source). Next: Phase 4 (collaboration) / Phase 5 (perf+polish) per §10.8.)
