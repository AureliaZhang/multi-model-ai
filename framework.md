...
> **Last Updated**: 2026-07-21 (v0.7.37 — Phase 3 start (§10.8 TC1 #3): **API keys encrypted at rest** — `utils/crypto.ts` AES-256-GCM (`enc:v1:` envelope), opt-in via `ENCRYPTION_KEY` (unset = plaintext passthrough = today's behaviour; legacy plaintext auto-detected on read), transparent decrypt at all 6 station-key read sites + encrypt on write, boot sweep upgrades existing plaintext in place; +10 tests → 116; see §12 change log. Phase 3 remaining: cost dashboard + per-user quota (FE-B, need product input).)
...
- **Phase 3 — "see & control spend":** admin usage + **cost** dashboard (FE-B; data already in `api_usage_logs`), per-user quota/budget (FE-B + TC1), API-key encryption at rest (TC1 #3). Effort: M. **🟡 In progress:** API-key encryption at rest **✅ done (v0.7.37)**; cost dashboard + per-user quota still to do (both FE-B — need product input: cost/pricing source, quota policy).
