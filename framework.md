...
> **Last Updated**: 2026-07-24 (v0.7.43 — **Phase 4 (FE-B): shared persona / prompt library** — `persona_library` table (migration v3) + `GET/POST/PUT/DELETE /api/personas` (`requireAuth`; team-visible read/use, creator-or-admin edit/delete via `canModifyPersona`, +6 tests → server 144) + `personaStore` and a team-library section inside the existing `SystemPromptModal` (one-click apply, save-current-as-persona, inline rename/delete for own). server 144 / client `tsc -b`+`vite build`+eslint (0 err) clean. **Phase 4 remaining: team-vs-private file library only.**)
...
- **Phase 4 — "nice to use together":** shared prompt/persona library (FE-B), team-vs-private file library (FE-B + TC0 #4), message actions copy/regenerate/edit (FE-A), conversation search (FE-A). Effort: M–L (splittable). **🟡 In progress:** message actions **✅ done (v0.7.41)**; conversation search **✅ done (v0.7.42)**; shared prompt/persona library **✅ done (v0.7.43)**. Remaining: team-vs-private file library.
