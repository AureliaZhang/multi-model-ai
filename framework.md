...
> **Last Updated**: 2026-07-21 (v0.7.38 — Phase 3 cont. (FE-B usage dashboard, **first client change** of the team-readiness effort): admin **usage dashboard** — new `GET /api/usage/summary` (`computeUsageSummary`, pure/testable) per-user + per-model token/request/error aggregation; `UsageLogsPage` gains a collapsible summary (two tables) above the raw log. Token-based, **no $ yet** (owner chose usage-first; pricing source TBD). +5 tests → server 121; client `tsc -b`+`vite build`+eslint clean. Remaining Phase 3: per-user monthly token quota (hard block).)
...
- **Phase 3 — "see & control spend":** admin usage + **cost** dashboard (FE-B; data already in `api_usage_logs`), per-user quota/budget (FE-B + TC1), API-key encryption at rest (TC1 #3). Effort: M. **🟡 In progress:** API-key encryption at rest **✅ done (v0.7.37)**; usage dashboard (token-based, no $) **✅ done (v0.7.38)**; **per-user monthly token quota (hard block) — next**; $ cost on the dashboard deferred (needs pricing source).
