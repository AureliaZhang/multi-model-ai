...
2. **自动蒸馏学习**（v0.7.73 计划）— 对话告一段落后 AI 自动提炼结论/决定/偏好存入记忆库（代替只存原始消息），复用 kbSummarizer 的内部调用基建。
3. **聊天内联网搜索**（v0.7.74 计划）— 管理员配置搜索 API key（加密存储），聊天按条开启「联网」，检索结果+来源注入上下文；离线开发机只能测流程，部署后生效。
