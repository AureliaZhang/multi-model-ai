...
> **Last Updated**: 2026-07-24 (v0.7.44 — **Phase 4 COMPLETE (FE-B): team-vs-private file library** — `file_library.visibility` (migration v4; default `private`, existing files migrated to `team` to preserve current behaviour), `GET /api/files?scope=mine|team` (own files browsable by folder / flat team-shared list), `canSeeFile` gate on file detail + chunks, `PATCH /api/files/:id/visibility` (owner/admin), and `filterVisibleFileIds` on **both** RAG paths (chat injection + `POST /search`) so a member can't read/forge another's private file. FileBrowser gets a 我的/团队 scope toggle + per-file 🔒/👥 badge & toggle. +12 tests → server 156 / client `tsc -b`+`vite build`+eslint (0 err) clean. **Phase 4 done — all four items shipped (message actions, conversation search, persona library, file privacy).**)
