...
> **Last Updated**: 2026-07-23 (v0.7.42 — **Phase 4 (FE-A): conversation search** — `GET /api/conversations/search?q=` (`searchConversations`, title LIKE OR message-content EXISTS, list-scoped, LIKE-wildcards escaped, +6 tests → server 138) + a debounced search box in `Sidebar` that swaps the list for results while typing. client `tsc -b`+`vite build`+eslint (0 err/0 warn) clean. Phase 4 remaining: shared prompt/persona library, team-vs-private files.)
...
- **Phase 4 — "nice to use together":** shared prompt/persona library (FE-B), team-vs-private file library (FE-B + TC0 #4), message actions copy/regenerate/edit (FE-A), conversation search (FE-A). Effort: M–L (splittable). **🟡 In progress:** message actions **✅ done (v0.7.41)**; conversation search **✅ done (v0.7.42)**. Remaining: shared prompt/persona library, team-vs-private file library.
